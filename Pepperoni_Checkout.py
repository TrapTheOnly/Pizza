...
class Checkout:
    ...
    pizza = Pizza.PizzaBuilder('Pepperoni')
    frame = 0
    ...
    def create_frame(self, pizza_ex):
        self.pizza = pizza_ex
        self.frame = Tk()
        self.frame.geometry("420x700+70+0")
        self.frame.title("Pepperoni Pizza Checkout")
    ...
    def checkout_func(self):
        ...
        self.chicken_button = Button(self.frame, text='Remove Chicken', 
                                    bg = '#f2aca5', fg = 'black', 
                                    width = 20, height = 3,
                                    font = 'Times 12 bold',
                                    activebackground = 'red', command = self.remove_chicken)
        self.chicken_button.grid(row = 9+self.rows_count, column = 2)
        self.space = Label(self.frame, text = '\n\n', bg='#f2aca5')
    ...
    def remove_chicken(self):
        if 'Chicken' in self.pizza.extentions_list:
            self.pizza.remove_extention('Chicken')
            for widget in self.all_children(self.frame):
                widget.destroy()
            self.checkout_func()
        else:
    ...
    def get_price(self):
        ...
        self.label_price = Label(self.frame, text = price, bg = '#f2aca5',
